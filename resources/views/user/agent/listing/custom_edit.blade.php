...
@else
    <input type="hidden" name="is_popular" value="{{$listing->is_popular ?? ''}}">
@endif
                                <div class="col-sm-12">
                                    <div class="mb-2">
...
<script>
    ...
    $("#form-action-btn").on('click', function() {
        ...
        // --- NOVA LOGIKA ZA TYPE (IS_POPULAR) ---
        var listing_is_popular = $("#is_popular").val();

        // 1. Upozorenje samo ako je Admin (ako je polje <select>) a nije ništa odabrao
        if($("#is_popular").is("select") && !listing_is_popular){
            warning('Listing Type is required');
        }

        // 2. Definiramo je li polje ispravno 
        // (Ispravno je ako NIJE select (agent), ili ako JE select ali ima vrijednost)
