...
@section('frontend_layout')
                ...
                <div class="col-12">
                    <div class="atn-search-filter-wrap">
                        <ul class="nav nav-pills atn-search-nav-pills" id="pills-tab" role="tablist">
                            @php
                                 $types = \App\Models\CustomType::where('status', 1)->orderBy('sorting', 'asc')->take(10)->get();
                                        ...
                                        @else
                                           <div class="atn-search-content">
                                                ...
                                                <div class="atn-single-search-item afNone">
                                                    ...
                                                    <select class="at-nice-select transparent2-nice-select mh-filter-select" name="city">
                                                        <option value="all">{{ get_phrase('Select City') }}</option>
                                                        @foreach ($dynamicCities as $index => $cityName)
                                                            @php
                                                                $city = App\Models\City::where('id', $cityName->city)->first();
                                                            @endphp
                                                            <option value="{{ $city->id }}">{{ $city->name }}</option>
   ...
   @php
    ...
    // Dohvati SVE aktivne tipove, sortirano po 'sorting' stupcu
    $allCustomTypes = \App\Models\CustomType::where('status', 1)
        ->orderBy('sorting', 'asc')
        ->get();

    // Izračunaj listing_count za svaki tip
    $sortedCustomTypes = $allCustomTypes->map(function ($type) use ($staticTypes, $customlistings) {
        $typeSlug = $type->slug ?? '';
        $tableMap = [
        ...
        if (in_array($typeSlug, $staticTypes)) {
            ...
            $listingCount = $tableName ? \DB::table($tableName)->where('visibility', 'visible')->count() : 0;
        } else {
