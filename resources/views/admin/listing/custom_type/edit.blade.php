...
<div style="background: yellow; border: 3px solid red; padding: 15px; margin-bottom: 20px; font-weight: bold; color: black;">
    <h3>🕵️ DETEKTIV ZA PAKETE</h3>
    @php
        $debug_user = auth()->id();
        $debug_sub = \App\Models\Subscription::where('user_id', $debug_user)->where('status', 1)->orderBy('id', 'DESC')->first();
    @endphp

    @if($debug_sub)
        @php
            $debug_pkg = \App\Models\Pricing::where('id', $debug_sub->package_id)->first();
        @endphp
        <p>1. ID Pretplate: {{ $debug_sub->id }}</p>
        <p>2. Ime Paketa: {{ $debug_pkg->name ?? 'Nepoznato' }}</p>
        <p>3. Cijena Paketa (price): '{{ $debug_pkg->price ?? 'Nema' }}'</p>
        <p>4. Plaćeni iznos (paid_amount): '{{ $debug_sub->paid_amount }}'</p>
        <hr>
        <p>ŠTO FUNKCIJA VRAĆA: 
            <span style="font-size: 20px; background: white;">
            {{ has_paid_subscription() ? 'TRUE (Plaćeno - PRIKAZUJEM)' : 'FALSE (Besplatno - SKRIVAM)' }}
            </span>
        </p>
    @else
                            ...
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    ...
                                    <select name="country" id="country" class="form-control ol-form-control ol-select2">
                                        ...
                                        @if($croatiaId)
                                        <option value="{{ $croatiaId }}" {{ $listing->country == $croatiaId ? 'selected' : '' }}> {{ get_phrase('Croatia') }} </option>
                                        @endif
                                    </select>
                                </div>
                            </div>
    ...
    <script>
        ...
        $("#form-action-btn").on('click', function() {
            ...
            if (listing_category && listing_title && listing_description && listing_visibility && listing_country && listing_city && listing_address && listing_post_code && is_popular) {
